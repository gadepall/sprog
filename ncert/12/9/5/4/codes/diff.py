...
x_values_np = np.array([x_values[i] for i in range(N) ]) #This code is converting the C-style arrays (x_values and y_values) created using ctypes into NumPy arrays
y_values_np = np.array([y_values[i] for i in range(N) ])

# Define the theoretical solution using the integrating factor method
def theoretical_solution(x):
    return 1 - x / (np.tan(x) + (1/np.cos(x))) 

# Generate x values for the theoretical solution (avoiding pi/2)
x_theory = np.linspace(0, np.pi/2 - 0.01, 500)
y_theory = theoretical_solution(x_theory)

# Plot both numerical and exact solutions on the same graph
plt.figure(figsize=(10, 7))
plt.plot(x_values_np, y_values_np, 'yo', label="sim", markersize=5)
