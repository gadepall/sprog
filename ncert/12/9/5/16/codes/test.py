...
def derivative(x,y,H):
    return H*x/y  # returns the differential equation
...
for i in range(1, 100):
    dybydx = derivative(x[i-1],y[i-1],H)
    x[i] = solve.generate_xpoints(c_double(x[i-1]), c_double(h),c_double(dybydx))  # function call to get x coordinates
    y[i] = solve.generate_ypoints(c_double(y[i-1]), c_double(h))  # function call to get y coordinates
...
y_theoretical = np.sqrt(x_theoretical)  
