...
// Function to calculate dx/dy for the equation y * e^(x/y) dx/dy = x * e^(x/y) + y^2
float dx_dy(float x, float y) {
    // Avoid division by zero or near-zero values in y
    if (fabs(y) < 1e-6) {
...
void points(float x_0, float y_0, float y_end, float h, float *x_points, float *y_points, int steps) {
    ...
    for (int i = 0; i < steps; i++) {
        ...
        // Calculate the next y and x values
        float y_n1 = y_n + h;
