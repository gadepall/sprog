...
gradient_lib = ctypes.CDLL('./main.so')
...
# Parameters
x_start = 0.1       # Start of range
x_end = 10.0         # End of range
...
print(f"Minima found at x = {min_coords[0]}, y = {min_coords[1]}")
print(f"Maxima found at x = {max_coords[0]}, y = {max_coords[1]}")

# Plot the function and the gradient descent result
plt.plot(x_vals, y_vals, label='function')
plt.scatter(min_coords[0], min_coords[1], color='red', label='local minima')
plt.scatter(max_coords[0], max_coords[1], color='green', label='local maxima')
plt.plot(-x_vals, -y_vals, label='function')
