...
# dll linking
dll = ctypes.CDLL('./trapezoidal.so')
...
lineArea = dll.areaTrapezoidLine(0, 4, 0.0001)
circArea = dll.areaTrapezoidCirc(4,4*math.sqrt(2), 0.001)
...
ptsLine = dll.lineGet(n, -5, 5, a, b, c) 

# getting points for the circle x^2 + y^2 = 4
O = (0, 0) # center (0, 0)
r = 4*math.sqrt(2) # radius
...
m = 100
ptsFill = dll.fillSector(m, n, math.pi/4, O[0], O[1], r)
coordsFill = []
...
plt.scatter(coords_plot_fill[0,:], coords_plot_fill[1,:], marker=".", label = "$Area$", color="orange")
...
plt.scatter(coords_plot_line[0,:], coords_plot_line[1,:], marker=".", label = "Line", color="green")
...
plt.scatter(coords_plot_circ[0,:], coords_plot_circ[1,:], marker=".", label = "Circle", color="blue")
