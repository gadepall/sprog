...
lu_lib = ctypes.CDLL('./code.so')

# Define matrix and vector size
n = 2

# Define the system of equations:
A = np.array([[2, 1], [3, 2]], dtype=np.float64)
...
# Define the equations to plot
x_vals = np.linspace(-5, 5, 100)
y1 = (5 - 2 * x_vals)  # Equation 1: 2x + y = 5  => y = 5 - 2x
y2 = (8 - 3 * x_vals) / 2  # Equation 2: 3x + 2y = 8  => y = (8 - 3x)/2

# Plot the equations
plt.figure(figsize=(8, 6))
plt.plot(x_vals, y1, label="2x + y = 5", color="blue")
plt.plot(x_vals, y2, label="3x + 2y = 8", color="red")
