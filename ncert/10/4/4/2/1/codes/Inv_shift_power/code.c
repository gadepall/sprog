...
void GaussianElimination(double mat[MAX_SIZE][MAX_SIZE], double *x, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Check for singular matrix and add a small perturbation
        if (fabs(b[i][i]) < EPS) {
            b[i][i] += EPS; // Add a small perturbation to avoid singularity
        }
...
double eval_poly(double *coeffs, int degree, double x) {
    double result = 0;
...
double eval_derivative(double *coeffs, int degree, double x) {
    double result = 0;
    for (int i = 0; i < degree; i++) {
...
// Function to calculate the shift using Gershgorin Circle Theorem
double calculate_shift(double *coeffs, int degree) {
    double radius = 0.0;
    for (int i = 0; i < degree; i++) {
        radius += fabs(coeffs[i]);
    }
    return radius + 1.0; // Shift outside the Gershgorin circle
}

// Function to normalize polynomial coefficients

// Inverse power iteration to compute the minimum eigenvalue
double inv_power_iter(double mat[MAX_SIZE][MAX_SIZE], double *eigvec, int n, double shift) {
    double x[MAX_SIZE];
    double eigval = 0, prev_eigval = eigval;
    int iter = 0;

    // Apply the shift to the matrix
    for (int i = 0; i < n; i++) mat[i][i] -= shift;
    ...
    return eigval + shift; // Add the shift back to get the actual eigenvalue
}
...
int main() {
    ...
    while (degree > 1) {
        ...
        // Calculate the shift based on the polynomial's degree
        double shift = calculate_shift(coeffs, degree);
        printf("Calculated Shift: %.6f\n", shift);

        printf("Debug: Starting inverse power iteration\n");
        double root = inv_power_iter(mat, eigvec, degree, shift);
