...
void lcd_display_biline(int pos1, char* buf1, int pos2, char* buf2){
    lcd_clear();
    ...
    lcd_cmd(0x80);
    if(pos1 > 16) lcd_message(buf_cpy + pos1 - 15);
    else lcd_message(buf_cpy);

    lcd_cmd(0xC0);
    if(pos2 > 16) lcd_message(buf2 + pos2 - 15);
    else lcd_message(buf2);
...
/* BUTTON FUNCTIONS */
void button_capturer(int* button_x, int* button_y, int* debounce) {
...
char button_grid(int button_x, int button_y, int is_mode){
    /*
     * @ -> arcsin
     * # -> arccos
     * $ -> arctan
    ...
    if(is_mode) return button_grid_mode[button_y][button_x];
    return button_grid[button_y][button_x];
}
...
int main(void){
 ...
 // initialize LCD controller
    ...
    while(1){
        button_capturer(&button_x, &button_y, &debounce);

        if(button_x != -1 && debounce == 0){
            char mapped_button = button_grid(button_x, button_y, is_mode);
            ...
            if(mapped_button == '='){
                ...
                pos2 = 16;
                debounce += 1;
                lcd_display_biline(pos1, buf1, pos2, buf2);
                _delay_ms(50);
                continue;
            }
